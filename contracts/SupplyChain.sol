...
/* Create a modifer that checks if the msg.sender is the owner of the contract */
  ...
  modifier forSale(uint sku){
    require(items[sku].state == State.ForSale, "Not on sale phase");
    _;
  }
  modifier sold(uint sku){
    require(items[sku].state == State.Sold, "Not yet sold");
    _;
  }
  modifier shipped(uint sku){
    require(items[sku].state == State.Shipped, "Not yet shipped");
    _;
  }
  modifier received(uint sku){
    require(items[sku].state == State.Received, "Not yet received");
    _;
  }
